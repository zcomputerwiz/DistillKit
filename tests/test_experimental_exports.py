...
import distillkit.residual_gate as legacy_residual_gate
...
import distillkit.experimental.residual_gate as exp_residual_gate
...
def test_residual_gate_exports():
    assert exp.ResidualAdmissionGate is exp_residual_gate.ResidualAdmissionGate is legacy_residual_gate.ResidualAdmissionGate
    assert exp.ResidualGateHandle is exp_residual_gate.ResidualGateHandle is legacy_residual_gate.ResidualGateHandle
    assert exp.TrigramFamiliarity is exp_residual_gate.TrigramFamiliarity is legacy_residual_gate.TrigramFamiliarity
    assert exp.install_residual_gates is exp_residual_gate.install_residual_gates is legacy_residual_gate.install_residual_gates
    assert exp.remove_residual_gates is exp_residual_gate.remove_residual_gates is legacy_residual_gate.remove_residual_gates
    assert exp.residual_gates is exp_residual_gate.residual_gates is legacy_residual_gate.residual_gates
    assert exp.calibrate_gates is exp_residual_gate.calibrate_gates is legacy_residual_gate.calibrate_gates
    assert exp.attach_residual_gates is exp_residual_gate.attach_residual_gates is legacy_residual_gate.attach_residual_gates
    assert exp.gate_parameter_count is exp_residual_gate.gate_parameter_count is legacy_residual_gate.gate_parameter_count
    assert exp.ResidualGateCheckpointCallback is exp_residual_gate.ResidualGateCheckpointCallback is legacy_residual_gate.ResidualGateCheckpointCallback
    assert exp_residual_gate.FAMILIES == legacy_residual_gate.FAMILIES
    assert exp_residual_gate.family_features is legacy_residual_gate.family_features


def test_the_optimizer_still_recognises_the_gate_as_architecture():
    """The freeze and learning-rate splits find the gate by class; the move kept it."""
    from distillkit.optimizers import ResidualAdmissionGate

    assert ResidualAdmissionGate is exp_residual_gate.ResidualAdmissionGate
